                ...
                <div className="relative flex gap-5 sm:gap-8">
                  ...
                  <div className="relative z-10 flex-shrink-0 w-11 sm:w-12 flex items-start justify-center pt-[1.1rem]">
                    <div
                      className={cn(
                        ...
                        item.current
                          ? 'timeline-dot-active'
                          : 'bg-surface border-border hover:border-accent/60'
                      )}
                    />
                  </div>
                  ...
                  <div
                    className={cn(
                      ...
                      item.current
                        ? 'card-accent border-accent/20 bg-accent/[0.02]'
                        : 'card-accent'
                    )}
                  >
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-4">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          ...
                          {item.current && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-accent/10 border border-accent/25 text-accent text-[10px] font-bold uppercase tracking-wider">
                    ...
                    <ul className="space-y-2.5 mb-5">
                      {item.description.map((point, i) => (
                        <li key={i} className="flex items-start gap-3 text-text-secondary text-sm leading-relaxed">
                          <span className="w-1.5 h-1.5 rounded-full bg-accent/50 mt-[0.45rem] shrink-0" />
                          {point}
                        </li>
                      ))}
                    </ul>
                    ...
                    {roleTech[item.id] && (
                      <div className="flex flex-wrap gap-1.5 pt-4 border-t border-border">
                        {roleTech[item.id].map((tech) => (
                          <span
                            key={tech}
                            className="px-2.5 py-1 rounded-full bg-secondary border border-border text-text-secondary text-[11px] font-medium hover:border-accent/40 hover:text-text-primary transition-colors duration-200"
                          >
                            {tech}
                          </span>
                        ))}
